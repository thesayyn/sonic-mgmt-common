...
func CVL_LEVEL_LOG(level CVLLogLevel, format string, args ...interface{}) {
	...
	switch level {
	case INFO:
		log.V(lvl.DEBUG).Infof(format, args...)
	case WARNING:
		log.V(lvl.DEBUG).Infof(format, args...)
	case ERROR:
	...
	case INFO_API:
		log.V(lvl.DEBUG).Infof(format, args...)
	case INFO_TRACE:
		log.V(lvl.DEBUG+1).Infof(format, args...)
	case INFO_DEBUG:
		log.V(lvl.DEBUG+2).Infof(format, args...)
	case INFO_DATA:
		log.V(lvl.DEBUG+3).Infof(format, args...)
	case INFO_DETAIL:
		log.V(lvl.DEBUG+4).Infof(format, args...)
	case INFO_ALL:
		log.V(lvl.DEBUG+5).Infof(format, args...)
	}
